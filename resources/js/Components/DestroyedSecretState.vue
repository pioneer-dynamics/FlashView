...
interface Props {
    reason?: 'wrong-password' | 'unavailable';
}
...
const label = computed((): string => props.reason === 'wrong-password'
    ? 'Secret destroyed'
    : 'Secret unavailable');

const headline = computed((): string => props.reason === 'wrong-password'
    ? 'Wrong password — this secret has been permanently destroyed.'
    : 'This secret is no longer available.');

const body = computed((): string => props.reason === 'wrong-password'
