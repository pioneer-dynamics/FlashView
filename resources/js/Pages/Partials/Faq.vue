...
<template>
    <FlatActionSection v-if="!$page.props.jetstream.flash?.secret?.url" class="pt-4">
        ...
        <Faq question="What would be some of the use cases?">
            <ul class="list-disc list-inside">
                <li>For sending passwords, OTP, API keys, etc, or sharing your Netflix credentials with famliy.</li>
                <li>To confess to your secret crush.</li>
                <li>Tell your kids about your grandfather's secret treasure stash.</li>
                <li>Anything you can think off...</li>
            </ul>
        </Faq>
        <Faq question="How do you avoid harassment and other illegal uses of the system?">
        ...
        <Faq question="How is the metadata stored?">
        ...
        <Faq question="Are there any limits to the number of secret links I can generate?">
            We rate limit the number of times an anonymous user can generate links to avoid abuse. Currently, anonymous users can create {{ $page.props.config.secrets.rate_limit.guest.per_minute }} links per minute and {{ $page.props.config.secrets.rate_limit.guest.per_day }} links per day. <Link class="underline text-gamboge-300" :href="route('login')">Login</Link> or <Link class="underline text-gamboge-300" :href="route('register')">create a free account!</Link> to increase the limit. Authenticated users on the free plan can create {{ $page.props.config.secrets.rate_limit.user.per_minute }} links per minute. Check out our <Link class="underline text-gamboge-300" :href="route('plans.index')">paid plans</Link> if you need to send larger messages.
        </Faq>
    </FlatActionSection>
</template>