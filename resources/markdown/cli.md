...
1. **P2P WebRTC (direct):** If both machines are running simultaneously, the sender attempts a direct peer-to-peer connection via WebRTC data channels. If ICE negotiation succeeds, the encrypted payload is delivered machine-to-machine without touching your storage — typically faster, especially on the same network.
2. **S3 relay (fallback):** If P2P is unavailable (e.g., NAT traversal fails, receiver is not running yet, or the 10-second P2P window times out), the sender uploads the encrypted payload to S3. The receiver polls for the upload and downloads it when ready. This path works across any network and does not require both machines to be online simultaneously.
...
When both machines run simultaneously, the transfer completes peer-to-peer and the sender prints `Transfer complete.` When the receiver is started after the sender, or P2P fails, the sender prints `Transfer ready. Run 'flashview pipe' on the receiving machine.` and the receiver downloads from the relay.
