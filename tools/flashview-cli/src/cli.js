 ...
 * @param {{ verbose: boolean, chunkSize: number, expiresIn: number }} options
 */
async function runPipeSender(client, options) {
    ...
    let session;
    try {
        session = await client.createPipeSession(sessionId, 'relay');
    } catch (err) {
...
async function runPkiSetup(client, { hasSeed }) {
    ...
    if (!hasSeed) {
        // === RECEIVER ROLE: No local seed, wait for Machine A to send seed ===
        ...
        // Compute pairing code (sender is arg 1, receiver is arg 2)
        ...
        // Save the seed and mark pairing accepted
        ...
        // === SENDER ROLE: Has seed, pair with a waiting device ===
...
const pipeCmd = program
    ...
    .action(withErrorHandling(async (options) => {
        ...
        if (!process.stdin.isTTY) {
            // Sender mode: stdin is piped
            await runPipeSender(client, { verbose, chunkSize });
        } else {
            // Receiver mode: no piped input
