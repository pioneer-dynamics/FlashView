 ...
 * local Vue refs (decryptionFailed, decryptionPending), aligned CLI copy, and
 * a DestroyedSecretState.vue component consumed by SecretForm.vue.
 *
...
class PIO76Test extends TestCase
{
    ...
    public function test_destroyed_secret_state_component_exists_with_both_reasons(): void
    {
        ...
        $this->assertStringContainsString(
            ...
            $contents,
            'DestroyedSecretState.vue must show the "This secret is no longer available." headline for reason="unavailable" (expired / already-retrieved path).'
        );
        ...
        $this->assertStringContainsString(
            'Return to FlashView',
            $contents,
            'DestroyedSecretState.vue CTA must be "Return to FlashView" — a pure navigation affordance, not "Send a new secret link" (the recipient cannot create a substitute).'
        );
    ...
    public function test_secret_form_uses_local_state_and_imports_destroyed_component(): void
    {
        ...
        $this->assertStringContainsString(
            'decryptionPending',
            $contents,
            'SecretForm.vue must declare a decryptionPending reactive ref to close the async race window.'
        );
        ...
        $this->assertMatchesRegularExpression(
            '/v-if="[^"]*decryptionPending[^"]*"/',
            $contents,
            'SecretForm.vue must gate a "Decrypting…" affordance on decryptionPending so the misleading intermediate UI does not render.'
        );
